...
function DrawMultiLineString(x,y,lineheight,screenwidth,fontsize,inputstr)
   ...
   while (i<=lenInByte) 
    do
        ...
        --print(lenInByte)
        print('i , lenInByte',i,lenInByte,width)
        
        if width >=math.ceil(screenwidth/fontsize) or  i>=lenInByte then
...
function restart()
    ...
    eink.print( math.ceil( (height-string.len(" ")*20 )/2 ), width/2-32/2, " ", 0, 24)
...
function ota()
    ...
    -- 生成OTA的URL
    local iot_url = "http://zhaopy.com/api/site/firmware_upgrade"
            ...
            --if mqttc:subscribe(device_set) then
                ...
                while true do
                    ...
                    if r then
                        ...
                        --mytable["firmware"]=rtos.firmware()
